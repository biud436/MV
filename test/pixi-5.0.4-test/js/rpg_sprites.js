...
Spriteset_Map.prototype.initialize = function() {
    ...
    this._isReadyToTileset = false;
};

Spriteset_Map.prototype.createLowerLayer = function() {
    Spriteset_Base.prototype.createLowerLayer.call(this);
    this.createParallax();
    this.createShaderTilemap(function() {
      this._isReadyToTileset = true;
...
Spriteset_Map.prototype.update = function() {
    ...
    if(!this._isReadyToTileset) return;
    this.updateTileset();
    this.updateParallax();
    this.updateTilemap();
    this.updateShadow();
...
/** 
 * https://github.com/pixijs/pixi-tilemap/blob/master/demo/rpgmaker.js
 */
Spriteset_Map.prototype.createShaderTilemap = function(cb) {
    var tilesets = null;
    var tileset = null;
    ...
    if(this._tileset)
      tilesetNames= this._tileset.tilesetNames;
    
    tilesetNames.forEach(function(name) {
      if (name.length > 0) loader.add(name, 'img/tilesets/' + name + '.png');
    });
    
    ShaderTilemap.isReady = false;

    loader.load(function(loader, resources) {
        self._tilemap = new ShaderTilemap();
        for (var i=0; i<tilesetNames.length; i++) {
            var tex = resources[tilesetNames[i]] && resources[tilesetNames[i]].texture;
            self._tilemap.bitmaps.push(tex);
            if (tex) tex.baseTexture.mipmap = true;
        }
        while (self._tilemap.bitmaps.length>0 && !self._tilemap.bitmaps[self._tilemap.bitmaps.length-1]) {
            self._tilemap.bitmaps.pop();
        }        
        self._tilemap.flags = $gameMap.tilesetFlags();;
        self._tilemap.tileWidth = $gameMap.tileWidth();
        self._tilemap.tileHeight = $gameMap.tileHeight();
        self._tilemap.setData($gameMap.width(), $gameMap.height(), $gameMap.data());
        self._tilemap.horizontalWrap = $gameMap.isLoopHorizontal();
        self._tilemap.verticalWrap = $gameMap.isLoopVertical();
        self._tilemap.refresh();     
        
        cb.call(self);
        ShaderTilemap.isReady = true;
...
Spriteset_Map.prototype.loadTileset = function() {
    ...
    if (this._tileset) {
        ...
        var length = tilesetNames.length;
