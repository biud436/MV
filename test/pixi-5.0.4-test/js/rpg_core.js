...
Graphics._createRenderer = function() {
    ...
    try {
        ...
        this._renderer = new PIXI.Renderer({ width: width, height: height, transparent: true, view: view });
...
Tilemap.prototype.update = function() {
    ...
    for (var i=0; i<this.bitmaps.length;i++) {
        if (this.bitmaps[i] instanceof Bitmap) {
...
TilingSprite.prototype._renderWebGL_PIXI = PIXI.TilingSprite.prototype._render;
...
WindowLayer.prototype.initialize = function() {
    ...
    this._windowRect = new PIXI.Rectangle();
...
WindowLayer.prototype.render = function(renderer) {
    ...
    for (var i = 0; i < this.children.length; i++) {
        ...
        if (child.visible && child.openness > 0) {    
          if(child._isWindow) {
...
WindowLayer.prototype._maskWindow = function(window, shift) {
    ...
    if(window.mask) {
      this.removeChild(window.mask);
    }
    
    rect.x = this.x + window.x;
    rect.y = this.x + window.y + window.height / 2 * (1 - window._openness / 255);
