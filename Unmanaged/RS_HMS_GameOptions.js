...
(function () {
  ...
  RS.MessageSystem.Params.minMessageSpeed = parseInt(
    parameters['minMessageSpeed'] || 0
  );
  RS.MessageSystem.Params.maxMessageSpeed = parseInt(
    parameters['maxMessageSpeed'] || 10
  );
  RS.MessageSystem.Params.messageSpeedButtonName = String(
    parameters['messageSpeedButtonName'] || '메시지 속도'
  );
  RS.MessageSystem.Params.fontSizeButtonName = String(
    parameters['fontSizeButtonName'] || '글자 크기'
  );
  ...
  if (!Imported.RS_ScreenManager && Imported.RS_MessageSystem) {
    ...
    ConfigManager.readMessageSpeed = function (config) {
      ...
      if (value !== undefined) {
        return Number(value).clamp(
          RS.MessageSystem.Params.minMessageSpeed,
          RS.MessageSystem.Params.maxMessageSpeed
        );
      } else {
        return RS.MessageSystem.Params.textSpeed;
      }
    ...
    Window_MessageOptionsImpl.prototype = Object.create(
    ...
    Window_MessageOptionsImpl.prototype.addVolumeOptions = function () {
      ...
      this.addCommand(
        RS.MessageSystem.Params.messageSpeedButtonName,
        'messageSpeed',
        true,
        'messageSpeed'
      );
      this.addCommand(
        RS.MessageSystem.Params.fontSizeButtonName,
        'fontSize',
        true,
        'fontSize'
      );
