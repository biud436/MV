...
* 수 있습니다. 이름 윈도우는 폭과 높이를 알아서 맞춰줍니다 :
*
*   \이름<이벤트명>
*
...
*   \골드
*
* 팝업 메시지 창을 만들 수 있는 기능입니다. 0은 이 이벤트, -1은 플레이어입니다:
* 사이드-뷰 전투나 프론트-뷰 전투 등 맵 이외의 다른 씬에서는 사용할 수 없습니다.
* 전투는 일반 맵과는 구조가 상당히 다르기 때문에 전투에서도 말풍선을 추가하려면
* 아마 새로운 텍스트 코드를 추가해야 할 것입니다.
* (추가는 어렵지 않지만 전투말풍선, 아군말풍선, 적군말풍선? 사실 이름을 짓기가 어려워서 보류 중입니다)
*
* \말풍선[이벤트의 ID]
* \말풍선[0]
* \말풍선[-1]
*
* 중앙 또는 오른쪽에 텍스트를 정렬할 수 있습니다. 이 플러그인에서 정의하지 않은 텍스트
* 코드를 사용하면 정확한 위치에 정렬이 되지 않을 수 있습니다.
* 이런 경우, drawTextEx의 반환 값을 이용하면 완벽하지만, 전체적인 구조를 바꿔야 해서
* 수정하지 않고 있습니다.
*
*   \정렬자[1]
*   \정렬자[2]
...
*   \크기[숫자]
*
* 탭과 캐리지 리턴 기능이지만 캐리지 리턴 기능은 완벽하지 않으니 사용에 주의 바랍니다.
* 다시 말하면 나중에 삭제될 수도 있는 텍스트 코드입니다.
*
*   \t : 탭의 크기는 8 입니다.
*   \r : X를 시작 위치로 되돌립니다.
*
