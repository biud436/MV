...
(function () {
  ...
  RSMatch.defaultCancelButtonName =
  ...
  TextBox.prototype.onKeyDown = function (e) {
    ...
    if (lastKey) {
      if (
        newKeyData.keyCode === TextBox.ENTER &&
        lastKey.keyCode === TextBox.ENTER &&
        newKeyData.keyRelease - lastKey.keyRelease <= 50
      ) {
        console.warn('The key interval is too short!');
        e.preventDefault();
      }
    }

    this._lastkey = newKeyData;
  ...
  Window_KorNameEdit.prototype.drawActorFace = function (
    actor,
    x,
    y,
    width,
  ...
  Window_KorNameEdit.prototype.updateWindowWidth = function () {
    ...
    if (RSMatch.windowWidth === 'auto') {
      this.width = Math.max(
        Math.min(padding + faceWidth + textWidth, Graphics.boxWidth - padding),
        580
      );
    } else {
  ...
  Scene_KorName.prototype.commandInput = function () {
    ...
    this._callbackStack.push(function () {
      self.popScene();
    });
  };
