...
(() => {
    ...
    let parameters = RS.MoreEnemiesplugins.filter((i) => {
        return i.description.contains("<RS_MoreEnemies>");
    });

    parameters = parameters.length > 0 && parameters[0].parameters;

    RS.MoreEnemies.jsonParse = function (str) {
        const retData = JSON.parse(str, (k, v) => {
            try {
                return RS.MoreEnemies.jsonParse(v);
            } catch (e) {
                return v;
            }
        });
    ...
    RS.MoreEnemies.createMoreEnemies = function () {
        troopSettings.forEach((settings) => {
            const troopId = settings.troopId;
            const troop =
                RS.MoreEnemiesdataTroops && RS.MoreEnemiesdataTroops[troopId];
            if (troop) {
                settings.moreEnemies.forEach((event) => {
                    if (
                        RS.MoreEnemiesdataTroops[troopId].members instanceof
                        Array
                    ) {
                        RS.MoreEnemiesdataTroops[troopId].members.push(event);
                    }
                });
            }
        });
    ...
    Spriteset_Battle.prototype.setPositionForEnemies = function () {
        const troopId = RS.MoreEnemiesgameTroop._troopId;
