...
(function ($) {
  ...
  const utils = {
    ...
    async writeConfigFile() {
      ...
      await writeFile(
        path.join(rootFolder, 'data', this.configFile),
        contents,
        'utf8'
      )
        .then(val => {
      ...
      let req = https.get(url, options, res => {
        ...
        if (res.statusCode === 200) {
          ...
          res.on('data', c => {
            ...
            if (isValid) {
              console.log(
                `${filename} => ${((100.0 * cur) / len).toFixed(0)}% (${(
                  cur / total
                ).toFixed(2)} MB)`
              );
            }
  ...
  Scene_Base.prototype.handleTouchInsideWindow = function (
    ecurrentSceneName,
    e
  ) {
    e.interactive = true;
  ...
  Scene_Base.prototype.initWithWindowPosition = function () {
    ...
    if (this._windowLayer)
      this._windowLayer.children.forEach(e =>
        this.resetWindow(currentSceneName, e)
      );
  };
