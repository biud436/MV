...
Imported.RS_ScreenManager = '1.08';
 ...
 * @param Graphics Size
 *
 * @param Enable Default Graphics Size
 * @parent Graphics Size
 * @type boolean
 * @desc if this is enabled, the graphic will be fixed with specific size, not the screen size.
 * @default false
 * @on Enable
 * @off Disable
 *
 ...
 * 2017.07.01 (v1.0.8) - Added the features that can set the screen size and the graphic size separately.
 */
...
(function () {
  ...
  // Thsese parameters is only the test options.
  ...
  var defaultScreenSize = (function () {
  ...
  var isEnabledDefaultGraphicsSize = Boolean(parameters['Enable Default Graphics Size'] === 'true');

  var defaultGraphicsSize = (function () {
    var ptDefaultGraphicSize = String(parameters["Default Graphics Size"] || '1280 x 720').split(' x ');
    var ret = new Point(
      parseInt(ptDefaultGraphicSize[0]) || 1280,
      parseInt(ptDefaultGraphicSize[1]) || 720
    );
    return ret;
  })();

  // This variable checks whether all windows is resizing

  var isResizeAllWindows = Boolean(parameters["Resize All Windows"] === "true");

  // This can set the resolutions based on this array variable in all platform, not the Windows.
  ...
  Graphics.setScreenResize = function (newScr) {
    ...
    if(isEnabledDefaultGraphicsSize) {
      newScr.x = defaultGraphicsSize.x;
      newScr.y = defaultGraphicsSize.y;
    }
  ...
  SceneManager.initGraphics = function() {
    ...
    // if this is enabled, the graphic will be fixed with specific size, not the screen size.
    if(isEnabledDefaultGraphicsSize) {
      bw = config.getSize(defaultGraphicsSize.x)[0];
      bh = config.getSize(defaultGraphicsSize.x)[1];
    }

    Graphics.initialize(bw, bh, type);
    Graphics.boxWidth = bw;
    Graphics.boxHeight = bh;
    Graphics._orientation = orientation;
  ...
  Graphics._setupEventHandlers = function() {
    ...
    window.addEventListener("deviceorientation", function (evt) {
      var angle = Math.abs(Math.ceil(Math.cos(PIXI.DEG_TO_RAD * evt.beta) * 5.0) / 5.0);
      if(angle === 1.0) {
        Graphics._changeOrientation('landscape');
      } else {
        Graphics._changeOrientation('portrait');
      }
    });

  };

  Graphics._changeOrientation = function (orientation) {
    if(!Utils.isMobileDevice()) return;
    if(orientation !== Graphics._orientation) {
      ...
      //   sh = temp;
      // }
      var config = new ScreenConfig(sw, sh, orientation);
      var size = config.getSize(sw);
      Graphics._orientation = orientation;
      Graphics.setScreenResize(new Point(size[0], size[1]));
    }
  };
  ...
  Graphics._onWindowResize = function() {
    ...
    if(Utils.isNwjs()) {
      ...
      var cx = (window.screen.availWidth / 2) - (window.innerWidth / 2);
      var cy = (window.screen.availHeight / 2) - (window.innerHeight / 2);
      if(dx !== window.innerWidth) {
        var mx = (window.innerWidth - dx);
        var my = (window.innerHeight - dy);
        window.resizeTo(
          parseInt(window.innerWidth - mx + xPadding),
          parseInt(window.innerHeight - my + yPadding)
        );
