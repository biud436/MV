...
(function () {
  ...
  RS.Window_NumberInputImpl.jsonParse = function (str) {
    var retData = JSON.parse(str, function (k, v) {
      try {
        return RS.Window_NumberInputImpl.jsonParse(v);
      } catch (e) {
        return v;
      }
    });
  ...
  RS.Window_NumberInputImpl.Params.fontSize = parseInt(
    parameters['Font Size'] || 28
  );
  RS.Window_NumberInputImpl.Params.windowWidthEval =
    parameters['Window Width'] || '480';
  RS.Window_NumberInputImpl.Params.windowHeightEval =
    parameters['Window Height'] || 'this.fittingHeight(6);';
  RS.Window_NumberInputImpl.Params.lineHeight = parseInt(
    parameters['Line Height'] || 36
  );
  RS.Window_NumberInputImpl.Params.buttonWidth =
    parameters['Default Button Width'] || '42';
  RS.Window_NumberInputImpl.Params.backgroundImage =
    parameters['Background Image'];
  RS.Window_NumberInputImpl.Params.backgroundType = parseInt(
    parameters['Window Background Type'] || 2
  );
  RS.Window_NumberInputImpl.Params.passwordChar = '*';
  RS.Window_NumberInputImpl.Params.passwordMode = Boolean(
    parameters['Password Mode'] === 'true'
  );
  ...
  Window_NumberEditImpl.prototype.initialize = function (
    varId,
    maxLength,
    isNumber
  ) {
  ...
  Scene_NumberInput.prototype.prepare = function (
    variableId,
    maxLength,
    isNumber
  ) {
  ...
  Scene_NumberInput.prototype.createBackground = function () {
    ...
    if (RS.Window_NumberInputImpl.Params.backgroundImage === '') {
      ...
      this._backgroundBitmap = ImageManager.loadTitle1(
        RS.Window_NumberInputImpl.Params.backgroundImage
      );
  ...
  Scene_NumberInput.prototype.createEditWindow = function () {
    this._editWindow = new Window_NumberEditImpl(
      this._varId,
      this._maxLength,
      this._isNumber
    );
